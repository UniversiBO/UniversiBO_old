...
class ShowContribute extends UniversiboCommand 
{
	function execute()
	{
      ...
      'Se pensate che il servizio che offriamo sia utile e desiderate che continui a essere disponibile per tutti allora aiutateci a rendere questo portale ancora migliore.'));
        ...
        Le modalitа di collaborazione sono tante e ognuna richiede tempi 
        diversi. Eccovi un breve elenco di ciт che potreste fare per aiutarci:',
      '[list]
        [*]potreste occuparvi di aggiungere [b]contenuti[/b] al sito: 
          se avete molto tempo potreste scrivere alcune pagine altrimenti и 
        ...
        [*]potreste aiutarci con l\'attivitпа di [b]moderazione[/b] e 
        ...
        [*]e se non avete la possibilitа di utilizzare il computer potreste 
          comunque aiutarci attraverso le [b]attivitа offline[/b]: 
          spargere la voce ai tuoi amici dell\'esistenza del sito(piщ persone 
          lo frequenteranno, piщ persone potranno contribuirne alla sua 
	  ...
	  'Se quindi vi abbiamo convinto con queste poche e semplici parole e volete 
		...
		e vi contatteremo al piщ presto.'));
		...
		$template->assign('question_q1Answers', array('una giornata alla settimana o piщ;','poche ore alla settimana;','pochi minuti alla settimana;')); 
		...
		$template->assign('question_q3', 'Quali di queste attivitа pensi di poter svolgere (anche piщ di una scelta)?'); 
		$template->assign('question_q3AnswersMulti', array('attivitа off-line(contatti con i docenti o studenti, reperimento materiale...);','moderatore  
		...
		$template->assign('question_Privacy', 'Acconsento al trattamento dei miei dati personali ai sensi del d. lgs. n. 196/2003;'); 
		...
		if (array_key_exists('f3_submit', $_POST)  )
		{
			...
			{
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'Il form inviato non пїЅ valido','file'=>__FILE__,'line'=>__LINE__ ));
			...
			if ( strlen($_POST['f3_nome']) > 50 ) {
				Error::throwError(_ERROR_NOTICE,array('id_utente' => $user->getIdUser(), 'msg'=>'Il nome indicato puпїЅ essere massimo 50 caratteri','file'=>__FILE__,'line'=>__LINE__,'log'=>false ,'template_engine'=>&$template ));
				$f3_accept = false;
			}	
			...
			if ( strlen($_POST['f3_cognome']) > 50 ) {
				Error::throwError(_ERROR_NOTICE,array('id_utente' => $user->getIdUser(), 'msg'=>'Il cognome indicato puпїЅ essere massimo 50 caratteri','file'=>__FILE__,'line'=>__LINE__,'log'=>false ,'template_engine'=>&$template ));
				$f3_accept = false;
			}
			...
			if ( (strlen($_POST['f3_tel']) > 50)||!ereg('^([0-9]{1,50})$', $_POST['f3_tel'])) {
				Error::throwError(_ERROR_NOTICE,array('id_utente' => $user->getIdUser(), 'msg'=>'Il numero di cellulare indicato puпїЅ essere massimo 20 cifre','file'=>__FILE__,'line'=>__LINE__,'log'=>false ,'template_engine'=>&$template ));
				$f3_accept = false;
			}
			...
			if ( strlen($_POST['f3_mail']) > 50 ) {
				Error::throwError(_ERROR_NOTICE,array('id_utente' => $user->getIdUser(), 'msg'=>'L\' indirizzo e-mail indicato puпїЅ essere massimo 50 caratteri','file'=>__FILE__,'line'=>__LINE__,'log'=>false ,'template_engine'=>&$template ));
				$f3_accept = false;
			}
			...
			if ( strlen($_POST['f3_cdl']) > 50 ) {
				Error::throwError(_ERROR_NOTICE,array('id_utente' => $user->getIdUser(), 'msg'=>'Il corso di laurea indicato puпїЅ essere massimo 50 caratteri','file'=>__FILE__,'line'=>__LINE__,'log'=>false ,'template_engine'=>&$template ));
				$f3_accept = false;
			}
...
//			var_dump($mail);die();
			if(!$mail->Send()) Error::throwError(_ERROR_DEFAULT,array('msg'=>'Il questionario и stato salvato ma и stato impossibile inviare la notifica ai coordinatori', 'file'=>__FILE__, 'line'=>__LINE__));
			
			$template->assign('question_thanks',"Grazie per aver compilato il questionario, la tua richiesta и stata inoltrata ai ragazzi che si occupano del contatto dei nuovi collaboratori.\n Verrai ricontattatato da loro non appena possibile");
