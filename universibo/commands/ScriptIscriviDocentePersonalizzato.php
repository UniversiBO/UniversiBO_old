...
class ScriptIscriviDocentePersonalizzato extends UniversiboCommand {
	function execute() {
		...
		if (!array_key_exists('cod_doc', $_GET) || !array_key_exists('new_username', $_GET))
			die('Errore: mancano dei parametri');

		$res = & $db->query('SELECT cod_doc, nome_doc, email FROM docente2 WHERE cod_doc = '.$db->quote($_GET['cod_doc']).' AND cod_doc NOT IN (SELECT cod_doc FROM docente
		 WHERE 1=1)');
		if (DB :: isError($res))
		...
		if (User :: selectUserUsername($username) == false) {
			...
			//var_dump($pippo);
			$new_user = new User(-1, USER_DOCENTE, $username, User :: passwordHashFunction($randomPassword), $row[2], $notifica, 0, '', '', $fc->getAppSetting('
			defaultStyle'));
			...
			$mail->Body = "Gentile docente,\n"."Le abbiamo creato un account su UniversiBO.\n\n"."Il progetto portato avanti dalla comunità degli studenti dell'università di Bologna collaborando con le strutture di Ateneo, si propone l
