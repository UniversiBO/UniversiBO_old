...
class ShowContattoDocente extends UniversiboCommand {
	...
	function execute()
	{
		...
		if (!array_key_exists('cod_doc',$_GET) && !ereg( '^([0-9]{1,10})$' , $_GET['cod_doc'] ) ) 
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'L\'utente cercato non � valido','file'=>__FILE__,'line'=>__LINE__, 'template_engine' => & $template)); 
		...
		if (!$docente)
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'L\'utente cercato non � un docente','file'=>__FILE__,'line'=>__LINE__, 'template_engine' => & $template));
...
//		var_dump($table_collab); die;
		...
		if ( array_key_exists('f35_submit_report', $_POST)  )
		{
			...
			if ( !array_key_exists('f35_stato', $_POST) || ! array_key_exists( $_POST['f35_stato'], $f35_stati))
				Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'Il form inviato non � valido','file'=>__FILE__,'line'=>__LINE__, 'template_engine' => & $template));
