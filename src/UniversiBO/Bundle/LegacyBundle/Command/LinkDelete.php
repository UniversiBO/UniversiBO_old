...
class LinkDelete extends CanaleCommand {
	...
	function execute() {
		...
		{
			Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'L\'id del link richiesto non � valido','file'=>__FILE__,'line'=>__LINE__ ));
		}
			
		$link = & Link::selectLink($_GET['id_link']);
		if ($link === false)
			Error :: throwError(_ERROR_DEFAULT, array ('id_utente' => $user->getIdUser(), 'msg' => 'Il link richiesto non � presente su database', 'file' => __FILE__, 'line' => __LINE__));
		...
		if (array_key_exists('f30_submit', $_POST)  )
		{
			if($link->deleteLink())
			{
				$template->assign('f30_langAction',"Il link � stato eliminato correttamente");
				return 'success';
			}	
		}
