...
class ChangePassword extends UniversiboCommand
{
    function execute()
    {
        ...
        if ($user->isOspite())
        {
            Error::throwError(_ERROR_DEFAULT,array('id_utente' => $user->getIdUser(), 'msg'=>'La modifica della password non puo` essere eseguita da utenti con livello ospite.'."\n".'La sessione potrebbe essere scaduta, eseguire il login','file'=>__FILE__,'line'=>__LINE__));
        }
        ...
        $template->assignUnicode('changePassword_langInfoChangePassword','Per modificare la propria password inserire i dati relativi al proprio username e alla vecchia password.'."\n".'Nei campi successivi riscrivere due volte la nuova password che si è scelto per evitare errori di battitura.');
        $template->assignUnicode('changePassword_langHelp','Per qualsiasi problema o spiegazioni contattate lo staff all\'indirizzo [email]'.$fc->getAppSetting('infoEmail').'[/email].'."\n".
