...
class NewPasswordStudente extends UniversiboCommand
{
    ...
    public function execute()
    {
        ...
        $template
                ->assign('newPasswordStudente_langNewPasswordAlt',
                        'Recupera Password');
        ...
        if (array_key_exists('f5_submit', $_POST)) {
            ...
            if ($_POST['f5_ad_user'] == '') {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Inserire la e-mail di ateneo',
                                'file' => __FILE__, 'line' => __LINE__,
                ...
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Lo username di ateneo indicato puo` essere massimo 30 caratteri',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
                ...
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Non inserire il suffisso "@studio.unibo.it" nella email di ateneo',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
            } elseif (!eregi('^([[:alnum:]])+\.[[[:alnum:]]+$',
                    $_POST['f5_ad_user'])) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                ...
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
            } elseif (!User::activeDirectoryUsernameExists(
                    $_POST['f5_ad_user'] . '@studio.unibo.it')) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Non esiste alcun utente di Universibo registrato con la mail di ateneo '
                                        . $_POST['f5_ad_user']
                                        . '@studio.unibo.it' . ' ',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
            ...
            if ($_POST['f5_password'] == '') {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Inserire la password della e-mail di ateneo',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
                ...
                $q5_password = $f5_password = $_POST['f5_password'];

            //username
            if ($_POST['f5_username'] == '') {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Inserire il proprio username',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
                ...
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Nello username sono permessi fino a 25 caratteri alfanumerici con lettere accentate, spazi, punti, underscore',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
            } elseif (!User::usernameExists($_POST['f5_username'], true)) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Lo username richiesto non e` registrato da nessun utente',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
                $f5_accept = false;
            } else
                $q5_username = $f5_username = $_POST['f5_username'];
        ...
        if ($f5_accept == true) {
            ...
            if (!User::activeDirectoryLogin($f5_ad_user, 'studio.unibo.it',
                    $q5_password, $adl_host, $adl_port)) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'L\'autenticazione tramite e-mail di ateneo ha fornito risultato negativo',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
            ...
            if ($user->isEliminato()) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Lo username inserito non e` registrato da nessun utente',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
            ...
            if ($user->getADUsername() != $q5_ad_user) {
                Error::throwError(_ERROR_NOTICE,
                        array('id_utente' => $user->getIdUser(),
                                'msg' => 'Lo username inserito non corrisponde con la mail di ateneo precedentemente registrata',
                                'file' => __FILE__, 'line' => __LINE__,
                                'log' => false, 'template_engine' => &$template));
            ...
            $mail->Body = "Ciao \"" . $user->getUsername()
                    ...
                    . "Per accedere al sito utilizza l'indirizzo "
            ...
            $msg = "La nuova password e` stata registrata con successo ma non è stato possibile inviarti la password tramite e-mail\n"
            ...
            if (!$mail->Send())
                Error::throwError(_ERROR_DEFAULT,
                        array('id_utente' => $user->getIdUser(), 'msg' => $msg,
                                'file' => __FILE__, 'line' => __LINE__));
