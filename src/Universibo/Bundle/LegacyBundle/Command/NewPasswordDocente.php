    ...
    public function execute()
    {
        ...
        if (!$session_user->isAdmin())
            Error::throwError(_ERROR_DEFAULT,
                    array('id_utente' => $session_user->getIdUser(),
                            'msg' => 'La generazione di una nuova password dei docenti e` possibile solo ad utenti amministratori.'
                                    . "\n"
                                    . 'La sessione potrebbe essere scaduta, eseguire il login',
                            'file' => __FILE__, 'line' => __LINE__));

        $username_allowed = explode(';',
                $fc->getAppSetting('passwordDocentiAdmin'));
        if (!in_array($session_user->getUsername(), $username_allowed))
            Error::throwError(_ERROR_DEFAULT,
                    array('id_utente' => $session_user->getIdUser(),
                            'msg' => 'La generazione di una nuova password dei docenti e` possibile solo a '
                                    . implode(', ', $username_allowed) . "\n"
                                    . 'La sessione potrebbe essere scaduta, eseguire il login',
                            'file' => __FILE__, 'line' => __LINE__));
        ...
        if ($docente === false
                || ($docente->isDocente() == false
                        && $docente->isTutor() == false))
            Error::throwError(_ERROR_DEFAULT,
                    array('id_utente' => $session_user->getIdUser(),
                            'msg' => 'L\'id dell\'utente richiesto non appartiene ad un docente o ad un tutor',
                            'file' => __FILE__, 'line' => __LINE__));
        ...
        if ($f21_accept == true) {
            ...
            $mail->Body = ($docente->isDocente()) ? "Gentile docente,\n"
            ...
            if (!$mail->Send())
                Error::throwError(_ERROR_DEFAULT,
                        array('id_utente' => $session_user->getIdUser(),
                                'msg' => 'Attenzione!!! La password e` stata aggiornata su database ma potrebbe essersi verificato un errore durante la spedizione dell\'email!!'
