...
class ForumApi extends DoctrineRepository implements ForumApiInterface
{
     ...
     */
    private $defaultRanks = array(User::STUDENTE => 0,
            User::COLLABORATORE => 9, User::TUTOR => 10, User::DOCENTE => 11,
            User::PERSONALE => 12, User::ADMIN => 1);
    ...
    public function login(User $user)
    {
        ...
        $query = 'INSERT INTO ' . $this->table_prefix
                ...
                . $db->quote($sid) . ', ' . $user->getIdUser() . ', ' . time()
                . ', ' . time() . ', '
        ...
        $query = 'SELECT user_session_time FROM ' . $this->table_prefix
                . 'users WHERE user_id = ' . $user->getIdUser();
        $stmt = $db->executeQuery($query);
    ...
    public function insertUser(User $user, $password = null)
    {
        ...
        if ($groups != User::OSPITE && $groups != User::STUDENTE
    ...
    VALUES(' . $db->quote($user->getIdUser()) . ', 1, '
    ...
    public function addForumInsegnamentoNewYear($forum_id, $anno_accademico)
    {
        ...
        $stmt = $db->executeQuery($query);
        $vecchio_nome = $stmt->fetchColumn();
        $nuovo_nome = $generator->update($vecchio_nome);
    ...
    public function getLastPostsForum(User $user, $id_forum, $num = 10)
    {
        ...
        $ultimo_login = ($user->getUltimoLogin() == null
                || $user->getUltimoLogin() == '') ? 0 : $user->getUltimoLogin();
    ...
    ORDER BY max(p.post_id) DESC';
        ...
        $query = $db->getDatabasePlatform()->modifyLimitQuery($query, $num);
        $stmt = $db->executeQuery($query);
